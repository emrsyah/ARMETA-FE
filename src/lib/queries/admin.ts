...
export function useResolveReport() {
    const queryClient = useQueryClient()
    return useMutation({
        mutationFn: async ({ id_report, status }: { id_report: string; status: 'Resolved' | 'Ignored' }) => {
            const response = await api.patch(`${ADMIN_ENDPOINTS.RESOLVE_REPORT}/${id_report}/resolve`, { status })
            return response.data
        },
        onSuccess: () => {
