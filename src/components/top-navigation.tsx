...
export function TopNavigation() {
	...
	const handleSearch = (query: string) => {
		if (query.trim()) {
			// Update history
			const newHistory = [query.trim(), ...searchHistory.filter((h) => h !== query.trim())].slice(
				0,
				3
			);
			...
			<div className="flex items-center gap-4">
				<DropdownMenu>
					<DropdownMenuTrigger asChild>
						<Button
							variant="ghost"
							className="relative h-10 w-10 rounded-full p-0 overflow-hidden hover:ring-2 hover:ring-primary/20 transition-all group"
						>
							{isLoading ? (
					...
					<DropdownMenuContent align="end" className="w-56 p-1 mt-2">
						...
						<DropdownMenuItem
							onClick={() => navigate({ to: "/a/profile" as any })}
							className="gap-2 cursor-pointer py-2 text-sm font-medium"
						>
						...
						<DropdownMenuItem
							...
							className="gap-2 cursor-pointer py-2 text-sm font-medium text-destructive focus:text-destructive focus:bg-destructive/10"
						>
