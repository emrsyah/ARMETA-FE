							...
							<div className="flex flex-col gap-2">
								<div className="flex flex-wrap gap-2">
									...
									{lecturerName && (
										<Badge
											variant={"outline"}
											className="bg-indigo-500/5 border-indigo-500/20 text-indigo-600 font-medium"
										>
											Dosen: {lecturerName}
										</Badge>
									)}
								...
								<Link to="/a/ulasan/$ulasanId" params={{ ulasanId: id }} search={{ focus: false }}>
									<h3 className="text-xl font-bold line-clamp-3 cursor-pointer hover:underline break-all">
					...
					{normalizedFiles && normalizedFiles.length > 0 && (
						<div className="shrink-0 w-64 gap-1 rounded-lg overflow-x-hidden overflow-y-hidden">
							{normalizedFiles.length === 1 ? (
								...
								<div
									className={`grid gap-1 ${normalizedFiles.length === 2 ? "grid-cols-2" : "grid-cols-3 grid-rows-2"}`}
								>
											...
											{isImage(file) ? (
												...
												>
													...
													<span className="text-[10px] text-center truncate w-full">
														{index === 0 && normalizedFiles.length > 1 ? getFileName(file) : "PDF"}
													</span>
												</div>
											)}
										</div>
