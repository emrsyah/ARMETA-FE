...
import type { getTools } from "@/lib/agent/agent";
...
function ArmePage() {
	...
	const { data: initialMessagesData, isLoading: isLoadingMessages } = useQuery<{
		data: MyUIMessage[];
	}>({
						...
						<div className="flex flex-col my-24 justify-center items-center h-full space-y-6 px-4">
							...
							<div className="space-y-2 text-center max-w-md animate-in fade-in slide-in-from-bottom-4 duration-700">
								...
								<p className="text-muted-foreground leading-relaxed">
									Tanyakan apa saja dan saya akan membantu menjawab pertanyaan Anda seputar
									perkuliahan, dosen, dan akademik.
								</p>
							</div>
						</div>
					) : (
							...
							<Message key={message.id} from={message.role}>
								<MessageContent>
									{message.parts.map((part: any, idx: number) => {
										...
										if (
											part.type === "tool-get_similar_ulasan" ||
											part.type === "tool-search_ulasan_text"
										) {
											return (
												<Tool key={idx} className="max-w-2xl">
													<ToolHeader
														...
														title={
															part.type === "tool-get_similar_ulasan"
																? "Mencari Ulasan Serupa"
																: "Mencari Ulasan Berdasarkan Kata Kunci"
														}
													/>
													<ToolContent>
										...
										if (
											part.type === "tool-get_similar_forum" ||
											part.type === "tool-search_forum_text" ||
											part.type === "tool-get_forums_by_subject"
										) {
											let title = "Mencari Diskusi Forum";
											if (part.type === "tool-search_forum_text")
												title = "Mencari Forum Berdasarkan Kata Kunci";
											if (part.type === "tool-get_forums_by_subject")
												title = "Mencari Forum Berdasarkan Mata Kuliah";
														...
														{part.state === "output-available" ? (
															<div className="p-4 pt-0">
																<ReviewCard
																	...
																	isReply={
																		!!(
																			(part.output as any).id_reply || (part.output as any).id_forum
																		)
																	}
																/>
															</div>
										...
										if (
											part.type === "tool-get_all_lecturers" ||
											part.type === "tool-get_all_subjects"
										) {
											return (
												<Tool key={idx} className="max-w-2xl">
													<ToolHeader
																		...
																		{(part.output as any[]).map((item: any, i: number) => (
																			<tr key={i} className="border-b hover:bg-muted/30">
																				...
																				<td className="p-2 font-mono text-[10px]">
																					{item.id_lecturer || item.id_subject}
																				</td>
																			</tr>
																		))}
																	</tbody>
										...
										if (part.type?.startsWith("tool-")) {
											return (
												<Tool key={idx} className="max-w-2xl">
													<ToolHeader type={part.type as any} state={part.state as any} />
													<ToolContent>
														<ToolInput input={part.input} />
														{part.state === "output-available" && (
