...
import { ArrowRight, FileText, MessageSquare, Search as SearchIcon } from "lucide-react";
		...
		return (
			<div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-8 bg-linear-to-b from-transparent to-accent/5 rounded-3xl border border-dashed border-muted-foreground/20 m-4">
				...
				<p className="text-muted-foreground max-w-md mx-auto text-lg leading-relaxed">
					Temukan ulasan mata kuliah, informasi dosen, dan diskusi forum yang Anda butuhkan dengan
					mengetikkan kata kuncinya di atas.
				</p>
			</div>
		);
	}
			...
			{isLoading ? (
				...
				<Tabs value={activeTab} onValueChange={setActiveTab} className="w-full space-y-8">
					<TabsList className="inline-flex h-12 items-center justify-center rounded-xl bg-muted p-1 text-muted-foreground w-full sm:w-auto">
						<TabsTrigger
							value="all"
							className="rounded-lg px-6 text-sm font-bold transition-all data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm"
						>
					...
					<AnimatePresence mode="wait">
						<TabsContent
							value="all"
							key="all"
							className="space-y-16 focus-visible:outline-none outline-none border-none"
						>
							{/* Forum Preview */}
							<motion.section
	...
	return (
		<div className="flex flex-col items-center justify-center py-32 bg-muted/20 rounded-4xl border border-dashed border-muted-foreground/30 text-center px-6">
			...
			<p className="text-muted-foreground max-w-sm mx-auto mt-4 text-base leading-relaxed opacity-80">
				Kami tidak dapat menemukan {category === "forum" ? "forum" : "ulasan"} yang cocok dengan
				pencarian Anda. Coba gunakan kata kunci yang lebih umum.
			</p>
		</div>
	);
}
