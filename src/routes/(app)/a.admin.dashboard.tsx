...
import { BarChart3, Users, MessageSquare, Gavel, TrendingUp, UserCheck, UserX, Award } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {statCards.map((card) => (
          <Card key={card.title} className="border-none shadow-sm bg-white/50 backdrop-blur-sm overflow-hidden relative group">
          ...
          <CardContent className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trendData}>
                ...
                <Tooltip
                  contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.8)', backdropFilter: 'blur(4px)', border: 'none', borderRadius: '8px', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}
          ...
          <CardContent className="h-[300px] flex flex-col items-center justify-center">
            <ResponsiveContainer width="100%" height="80%">
              <PieChart>
                ...
                <Tooltip
                  contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.8)', backdropFilter: 'blur(4px)', border: 'none', borderRadius: '8px' }}
                />
              </PieChart>
            </ResponsiveContainer>
              ...
              {stats?.topLecturers?.map((lecturer: any, index: number) => (
                <div key={lecturer.id} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted-foreground">{lecturer.review_count} ulasan</span>
                    <div className="w-24 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-amber-500 rounded-full"
                        style={{ width: `${(lecturer.review_count / (stats.topLecturers[0]?.review_count || 1)) * 100}%` }}
                      ></div>
                    </div>
                  </div>
                </div>
